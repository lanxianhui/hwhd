...
<%
    String jingpin_count=MessageUtil.getMessage(session, "game_count");
    ...
    if ((null == (jingpin_count)) || (("".equals(jingpin_count))) || ("null"==jingpin_count))
	{
		jingpin_count	=	"0";
	     ...
	     {
	  	    String tep_jingpin_name=MessageUtil.getMessage(session, "game_"+(i+1));
	  	    String tep_jingpin_url=MessageUtil.getMessage(session, "game_"+(i+1)+"_url");
	  	    String tep_jingpin_imgurl=MessageUtil.getMessage(session, "game_"+(i+1)+"_img_url");
...
<title>游戏</title><style type="text/css">
*{margin:0; padding:0;}
...
//进入看吧
function doSelect(){
	window.location =  menu[menuPos].TURNURL;
}
