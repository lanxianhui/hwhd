...
function grabEvent(event) {
	...
	//	overlayTimer=setTimeout(function(){iPanel.overlayFrame.close();},2000);	
	} else {
		switch(keycode) {
			case 4:
			case 39://右
			...
			case 1108://频道
			 ...
			 if(strurl.indexOf("HD_channelPortal.jsp")>-1) return;
			...
			case 261: //静音
				...
				if(urlstr.indexOf("play.jsp")>-1||urlstr.indexOf("HD_playliveControl")>-1||urlstr.indexOf("HD_playTvodControl")>-1||urlstr.indexOf("HD_portal.jsp")>-1){
					...
					if(urlstr.indexOf("HD_portal.jsp")>-1){
						...
						$("muteDiv").style.width="100px";
						$("muteDiv").style.height="100px";
						$("muteDiv").style.left="1050px";
					}else{
						$("muteDiv").style.width="250px";
						$("muteDiv").style.height="96px";
						$("muteDiv").style.backgroundImage="url(images/vod/mute.png)"; 
						$("muteDiv").style.left="950px";
					//	alert("wwwwwwwwwww");
					}
			...
			case 613://首页
				code = "KEY_HOMEPAGE";
					...
					volumePage.close();
				//window.location = "http://10.50.103.10:33200/EPG/MediaService/hwhd/Category.jsp";
			/*
				var urlstr=window.location.href;
				if(urlstr.indexOf("HD_portal.jsp")>-1) return 0;
			
				//window.location = "Category.jsp";
				iPanel.overlayFrame.location="test.html?0";
				iPanel.overlayFrame.resizeTo(748,220);
				iPanel.overlayFrame.moveTo(274,250);
				return 0;*/
				break;
...
function forChannel(urlstr){
	clearTimeout(chanelValueTimer);
	forchangeChannel(channelValue);
		//alert(forChannelNum);
	if(forChannelNum){
		if($("ChannelPrompg")==null||typeof($("ChannelPrompg"))=="undefined"){
			creatChannelPrompg();
		}
		if(channelValue.length==1) channelValue="00"+channelValue;
		if(channelValue.length==2) channelValue="0"+channelValue;	
		$("ChannelPrompg").style.webkitTransitionDuration="500ms";
		$("ChannelPrompg").style.webkitTransform= "scale(1)";
		$("channelPrompgText").innerText="频道<"+channelValue+">不存在";
		setTimeout('$("ChannelPrompg").style.webkitTransform= "scale(0)"',4000);
	}
		//alert(forChannelNum);
		chanelValueTimer=setTimeout(function (){
				//showNoChannel(channelValue);
			iPanel.overlayFrame.location="ChannelNum.html?"+channelValue+"?"+urlstr;
...
function creatMuteDiv(){
		...
		mydiv.style.left="1050px"; 
		mydiv.style.top="36px"; 
		mydiv.style.textAlign="center";
		mydiv.style.webkitTransitionDuration="300ms";
		document.body.appendChild(mydiv);
}
...
function showPrompt(n){
		if($("USBprompgDiv")==null||$("USBprompgDiv")=="undefined"){
			creatPrompgDiv();
		}
		$("USBprompgDiv").style.webkitTransitionDuration="500ms";
		$("USBprompgDiv").style.webkitTransform= "scale(1)";
		n==0?$("prompgText").innerText="检测到移动设备":$("prompgText").innerText="移动设备已删除"
		setTimeout(function (){$("USBprompgDiv").style.webkitTransform= "scale(0)";},2000);
}
*/
var searchId=[];
var folder =[];
...
function searchResult(modifiers,m,x){
		...
		while(typeof(files)!="undefined"&&files.atEnd()==false){	
				...
				//iPanel.misc
				var Obj=new fileObj(fileNode.Name.split(".")[0],fileNode.Path);
